...
post_prob_commit123 <- tibble(post_prob = post_prob(commit_glmm1, commit_glmm2, commit_glmm3))
BF_commit_abandon <- post_prob_commit123$post_prob[[1]]/post_prob_commit123$post_prob[[3]]

loo_commit_123_table <- rbind()

loo_commit1 <- loo(commit_glmm1)
loo_commit2 <- loo(commit_glmm2)
loo_commit3 <- loo(commit_glmm3)
loo_commit123 <- loo_compare(loo_commit1, loo_commit2, loo_commit3)
...
post_prob_commit8910 <- tibble(post_prob = post_prob(commit_glmm8, commit_glmm9, commit_glmm10))
BF_commit_latency <- post_prob_commit8910$post_prob[[1]]/post_prob_commit8910$post_prob[[3]]

loo_commit8 <- loo(commit_glmm8)
loo_commit9 <- loo(commit_glmm9)
loo_commit10 <- loo(commit_glmm10)
loo_commit8910 <- loo_compare(loo_commit8, loo_commit9, loo_commit10)
...
post_prob_commit8910_surv <- tibble(post_prob = post_prob(commit_glmm8_surv, commit_glmm9_surv, commit_glmm10_surv))
BF_commit_surv <- post_prob_commit8910_surv$post_prob[[1]]/post_prob_commit8910_surv$post_prob[[3]]

loo_commit8_surv <- loo(commit_glmm8_surv)
loo_commit9_surv <- loo(commit_glmm9_surv)
loo_commit10_surv <- loo(commit_glmm10_surv)
loo_commit8910_surv <- loo_compare(loo_commit8_surv, loo_commit9_surv, loo_commit10_surv)
...
post_prob_commit111213 <- tibble(post_prob = post_prob(commit_glmm11, commit_glmm12, commit_glmm13))
BF_commit_takeleave <- (post_prob_commit111213$post_prob[[3]]/post_prob_commit111213$post_prob[[2]])

loo_commit11 <- loo(commit_glmm11)
loo_commit12 <- loo(commit_glmm12)
loo_commit13 <- loo(commit_glmm13)
loo_commit111213 <- loo_compare(loo_commit11, loo_commit12, loo_commit13)
...
post_prob_sharing123 <- tibble(post_prob = post_prob(sharing_glmm1, sharing_glmm2, sharing_glmm3))
BF_share <- post_prob_sharing123$post_prob[[1]]/post_prob_sharing123$post_prob[[2]]

loo_share1 <- loo(sharing_glmm1)
loo_share2 <- loo(sharing_glmm2)
loo_share3 <- loo(sharing_glmm3)
loo_share_123 <- loo_compare(loo_share1, loo_share2, loo_share3)
...
post_prob_help123 <- tibble(post_prob = post_prob(helping_glmm1, helping_glmm2, helping_glmm3))
BF_help_tendency <- post_prob_help123$post_prob[[1]]/post_prob_help123$post_prob[[2]]

loo_help1 <- loo(helping_glmm1)
loo_help2 <- loo(helping_glmm2)
loo_help2_mommatch <- loo_moment_match(helping_glmm2, loo_help2)
loo_help3 <- loo(helping_glmm3)
loo_help123 <- loo_compare(loo_help1, loo_help2_mommatch, loo_help3)
...
helping_glmm8 <- brm(helping_latency ~ age_group_string + location_string + first + gender_string +
                     ...
                     iter = 10000,
                     warmup = 2000,
                     control = list(adapt_delta = 0.999999, max_treedepth = 70),
                     save_pars = save_pars(all = TRUE),
                     cores = 4)
...
post_prob_help8910 <- tibble(post_prob = post_prob(helping_glmm8, helping_glmm9, helping_glmm10))
BF_help_latency <- post_prob_help8910$post_prob[[1]]/post_prob_help8910$post_prob[[2]]

loo_help8 <- loo(helping_glmm8)
loo_help9 <- loo(helping_glmm9)
loo_help10 <- loo(helping_glmm10)
loo_help8910 <- loo_compare(loo_help8, loo_help9, loo_help10)
...
post_prob_help8910_surv <- tibble(post_prob = post_prob(helping_glmm8_surv, helping_glmm9_surv, helping_glmm10_surv))
BF_help_surv <- post_prob_help8910_surv$post_prob[[1]]/post_prob_help8910_surv$post_prob[[2]]

loo_help8_surv <- loo(helping_glmm8_surv)
loo_help9_surv <- loo(helping_glmm9_surv)
loo_help10_surv <- loo(helping_glmm10_surv)
loo_help8910_surv <- loo_compare(loo_help8_surv, loo_help9_surv, loo_help10_surv)

##Table 4
table_commit_glmm1 <- fixed(commit_glmm1)
table_commit_glmm8 <- fixef(commit_glmm8)
table_commit_glmm13 <- fixef(commit_glmm13)
table_sharing_glmm1 <- fixef(sharing_glmm1)
